...
- **_Edicy.com_** [Search over elements](http://www.edicy.com/developer/code-examples/template-coding-examples/search-over-elements)
...
function loadCategoryRecipes(currentPage) {
  ...
  $.when(
    $.ajax({
      url:
        '/admin/api/site/elements.json?element_definition_id=' +
        recipesModelId +
        '&per_page=100&page=' +
        currentPage +
        '&page_id=' +
...
function displayRecipes(recipesList) {
  ...
  $(recipesList).each(function(index, recipe) {
    ...
    $('.recipes-listing ul').append(
      '<li class="' +
        lastClass +
        '"><a href="' +
        recipe.url +
        '"><span class="image-holder" id="' +
        imageHolderId +
        '">' +
        recipe.image +
        '</span>' +
...
function centerRecipeImages() {
  var imageWidth, imageHeight, portraitImage;
  ...
  if (portraitImage) {
    $img
      .css({
        ...
        top:
          squareDimensions / 2 -
          (imageHeight * (squareDimensions / imageWidth)) / 2 +
          'px',
      })
      .hide()
      .fadeIn('slow');
    ...
    $img
      .css({
        ...
        left:
          squareDimensions / 2 -
          (imageWidth * (squareDimensions / imageHeight)) / 2 +
          'px',
      })
      .hide()
      .fadeIn('slow');
...
function filterRecipes(elements) {
  var selectedOption = {
    ...
    selectedRegexp = {
      ingredient:
        '^' + selectedOption.ingredient + '| // ' + selectedOption.ingredient,
      function:
        '^' + selectedOption.function + '| // ' + selectedOption.function,
    },
    condition = '',
    recipes = [];
  ...
  switch (condition) {
    case 'if':
      $(elements).each(function(index, recipe) {
        if (
          typeof recipe.values.filter_ingredients === 'undefined' &&
          typeof recipe.values.filter_function === 'undefined'
        ) {
        } else {
          if (
            recipe.values.filter_ingredients.match(selectedRegexp.ingredient) &&
...
function calculateGdaValues() {
  ...
  jQuery('#gda-energy_cent').html(
    Math.round((portion_energy / 2000) * 100) + '%',
  );
  jQuery('#gda-protein_cent').html(
    Math.round((portion_protein / 50) * 100) + '%',
  );
  jQuery('#gda-carbon_cent').html(
    Math.round((portion_carbon / 270) * 100) + '%',
  );
